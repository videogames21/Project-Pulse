...
const selectedRubricId = ref(null)   // null = no rubric chosen

// Step 3 — criteria editing
const editableCriteria = ref([])     // copy of selected rubric's criteria
const criteriaEdited = ref(false)    // true if admin changed anything
...
function goToStep3() {
  // No rubric selected — skip criteria review, jump straight to confirm
...
function removeCriterion(index) {
  if (editableCriteria.value.length === 1) return  // always keep at least one criterion
...
<template>
  <AppLayout>
    ...
    <div class="flex items-center gap-2" style="margin-bottom:20px">
      ...
      <span class="muted" style="font-size:.85rem">Create Senior Design Section</span>
    </div>
    ...
    <div v-else-if="step === 2" class="card" style="max-width:620px">
      ...
      <div style="padding:20px">
        ...
        <template v-else>
          <p class="muted" style="font-size:.85rem;margin-bottom:12px">Select an existing rubric, or skip to create the section without one.</p>
          ...
          <!-- No rubric option — always visible -->
          <div
            class="card"
            style="padding:12px 16px;margin-bottom:8px;cursor:pointer;border:2px solid transparent;transition:border .15s"
          ...
          >
            <div class="flex items-center gap-3">
              ...
              />
              <div>
                <p style="font-weight:600;margin:0">No rubric (skip)</p>
                <p class="muted" style="font-size:.78rem;margin:2px 0 0">Assign a rubric later from the rubrics page.</p>
              </div>
            </div>
          </div>
    ...
    <div v-else-if="step === 3" class="card" style="max-width:700px">
      ...
      <div style="padding:20px">
        ...
        <p class="muted" style="font-size:.85rem;margin-bottom:16px">
          Review the criteria below. You may edit them — any changes will create a private copy of
    ...
    <div v-else-if="step === 4" class="card" style="max-width:620px">
      <div class="card-header">
        <h3 style="font-size:1rem;font-weight:700">Confirm Section Creation</h3>
      </div>
      <div style="padding:20px">
        ...
        <p class="muted" style="font-size:.85rem;margin-bottom:20px">
          Review the details below. Click <strong>Create Section</strong> to save.
        </p>
        ...
        <div class="card" style="padding:16px;background:var(--surface-2)">
          ...
          <div v-if="selectedRubricId === null" class="muted">None selected.</div>
